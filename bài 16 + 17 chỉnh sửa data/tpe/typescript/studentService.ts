...
// vd
// age();
export default class StudentSevice {
        ...
        for (const obj of this.data) {
            ...
            _tr += `
               <tr>
                ...
                <td>${xepLoai}</td>
                <td>
                    <button class= "bn btn-sm btn-primary btn-edit" data-id="${obj.code}">Sửa</button>
    ...
    findByCode1(code: string): any{
        let searchData = this.data.filter(function(obj){
            // return obj.code == code;// tìm đúng 100%
            return obj.code.toLowerCase() == code.toLowerCase();// tìm gần đúng
        });

        return searchData;
    }

    updateStd(code: string): any{
        let index = this.data.findIndex(function(obj){
            return obj.code == code;
        });
        var form1: any = document.getElementById('std_form');
        ...
        this.data[index].name = form1.name.value;// biến data đại diện cho mảng hiện tại đó là mã họ tên ..index là chỉ số của từng obj in array
        this.data[index].toan = parseFloat(form1.toan.value);
        this.data[index].ly = parseFloat(form1.ly.value);
        this.data[index].hoa = parseFloat(form1.hoa.value);
        let jsonData = JSON.stringify(this.data);
        localStorage.setItem('Student_data', jsonData);
        // splice là hàm xóa phần tử của mảng
        this.showListToTable();
